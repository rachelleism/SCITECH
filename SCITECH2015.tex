...
\end{center}
 ...
 plane of the nozzle ($\rho=2.04kg/m^{3}$, $U=22m/s$, $P=171427Pa$) to
%???? Why specify rho, U and P at the entrance, but rho U and T at the exit???
 ...
 mounted. 
% ?? Is this next sentence still valid?  It gives the impression that
% you are not solving the nozzle itself
The velocity profile at the entrance to the nozzle is that
 of a uniform flow (zero at the wall and freestream everywhere else).
 ...
 thickness has been estimated to be $???mm$).  Previous studies have
...
of $100\%$ results in the actuators being on all the time.  
%%??? Please make sure this is accurate.  You are responsible!
The experimental duty cycle varies with frequency, since the arc
strike lasts a fixed time.  The simulations on the other hand can
maintain the duty cycle independent of frequency.  Since the actuator
...
($1.97\%$ for $St_{DF}=0.25$) in order to obtain control authority.
...
excitation frequencies\cite{???}
%?? Cite your paper here
...
dynamics. The structure dynamics that produce the intricate waveforms
in Fig. \ref{phase} can not be easily obtained in experiments, since
that would require simultaneous PIV and pressure array data
acquisition.
%?? Is this above correct Michael (MC)?
...
%?? It should be "at" a phase, rather than "in" a phase, right??
...
%?? In general, write Fig.~\ref{???} rather than Fig \ref{???}.  The
%"~" makes sure that "Fig" and the number are together and you don't
%get an orphan number at the beginning of the line.???
...
actuation period apart. The $St_{DF}=0.05$ case exhibits a large peak
at about $\Delta \phi=-0.15(2\pi)$ which has a PID 
%?? What is PID??
like decay on either
